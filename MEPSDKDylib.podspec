Pod::Spec.new do |s|
  s.name        = "MEPSDKDylib"
  s.version     = "10.4.2"
  s.summary     = "Dynamic lib for MEPSDK"

  s.description = <<-DESC
MEP(Moxtra Engagement Platform) is a SDK or platform for partners to intergrate Moxtra service more easily.
  DESC

  s.homepage = "https://www.moxtra.com"
  s.license  = { :type => "MIT" }
  s.author   = { "john.hu" => "john.hu@moxtra.com" }

  s.platform      = :ios, "13.0"
  s.swift_version = "5.0"

  # SHA256: 4053c37b4ffd3bb0871953a19b3cac95b5582931286cf0df993aefb5dfb6efec
  s.source = {
    :http => "https://mxrepo.moxo.com/repository/customer-raw-releases/mepsdk/dynamic/Dynamic_MEPSDK_iOS_v10.4.2.zip"
  }

  s.vendored_frameworks = "MEPSDK.xcframework"
end
